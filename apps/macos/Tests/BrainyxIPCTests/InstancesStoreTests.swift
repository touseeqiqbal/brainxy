...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: BrainyxProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: BrainyxProtocol.AnyCodable] = [
            "presence": .init([BrainyxProtocol.AnyCodable(entry)]),
        ]
        let payload = BrainyxProtocol.AnyCodable(payloadMap)
