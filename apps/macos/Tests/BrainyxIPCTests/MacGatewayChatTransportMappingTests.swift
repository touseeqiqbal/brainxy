...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: BrainyxProtocol.AnyCodable(["ok": BrainyxProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: BrainyxProtocol.AnyCodable(["ok": BrainyxProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = BrainyxProtocol.AnyCodable([
            "runId": BrainyxProtocol.AnyCodable("run-1"),
            "sessionKey": BrainyxProtocol.AnyCodable("main"),
            "state": BrainyxProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: BrainyxProtocol.AnyCodable(["a": BrainyxProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
