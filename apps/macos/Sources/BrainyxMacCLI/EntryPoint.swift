...
struct BrainyxMacCLI {
    static func main() async {
        ...
        default:
            fputs("brainyx-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      brainyx-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      brainyx-mac discover [--timeout <ms>] [--json] [--include-local]
      brainyx-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      brainyx-mac connect
      brainyx-mac connect --url ws://127.0.0.1:18789 --json
      brainyx-mac discover --timeout 3000 --json
      brainyx-mac wizard --mode local
    """)
}
