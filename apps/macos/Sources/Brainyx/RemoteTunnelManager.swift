...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.brainyx", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Brainyx run already has an SSH listener on the expected port (common after restarts),
