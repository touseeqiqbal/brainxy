...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: BrainyxProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.brainyx", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: BrainyxKit.AnyCodable]()) {
                $0[$1.key] = BrainyxKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: BrainyxProtocol.AnyCodable?) -> [String: BrainyxProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: BrainyxProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: BrainyxKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: BrainyxProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("brainyx.control.heartbeat")
    static let controlAgentEvent = Notification.Name("brainyx.control.agent")
}
