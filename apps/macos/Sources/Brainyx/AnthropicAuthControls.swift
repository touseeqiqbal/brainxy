...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: BrainyxOAuthStore.AnthropicOAuthStatus = BrainyxOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: BrainyxOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = BrainyxOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = BrainyxOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try BrainyxOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Brainyx can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: BrainyxOAuthStore.AnthropicOAuthStatus,
