...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["BRAINYX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/brainyx")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("brainyx-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("brainyx-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/brainyx/ (rolling or stdout), or nil if none exist.
