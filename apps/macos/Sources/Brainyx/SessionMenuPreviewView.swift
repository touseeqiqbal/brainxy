...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.brainyx", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> BrainyxSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: BrainyxSessionPreviewEntry,
    ...
    private static func cache(payload: BrainyxSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: BrainyxChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [BrainyxKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [BrainyxKit.AnyCodable]) -> [BrainyxChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(BrainyxChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: BrainyxChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: BrainyxChatMessage) -> Bool {
    ...
    private static func toolNames(for message: BrainyxChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: BrainyxChatMessage) -> String? {
