...
enum BrainyxLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = BrainyxOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = BrainyxFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        BrainyxLogging.bootstrapIfNeeded()
        let label = BrainyxLogging.makeLabel(subsystem: subsystem, category: category)
...
struct BrainyxOSLogHandler: LogHandler {
...
struct BrainyxFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = BrainyxLogging.parseLabel(self.label)
