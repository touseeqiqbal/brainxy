...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.brainyx/brainyx.json. Prefer BRAINYX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.brainyx/brainyx.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.brainyx/brainyx.json. Restart the gateway to apply."
        }
