...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.brainyx", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: BrainyxPeekabooBridgeServices?
    private static var brainyxSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Brainyx", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = BrainyxPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.brainyxSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.brainyxSocketPath, privacy: .public)")
    }
...
@MainActor
private final class BrainyxPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.brainyx.peekaboo")
