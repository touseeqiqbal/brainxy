...
enum CommandResolver {
    private static let projectRootDefaultsKey = "brainyx.gatewayProjectRootPath"
    private static let helperName = "brainyx"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let brainyxEntry = root.appendingPathComponent("brainyx.mjs").path
        if FileManager().isReadableFile(atPath: brainyxEntry) { return brainyxEntry }
        let binEntry = root.appendingPathComponent("bin/brainyx.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/brainyx")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let brainyxPaths = self.brainyxManagedPaths(home: home)
        if !brainyxPaths.isEmpty {
            extras.insert(contentsOf: brainyxPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + brainyxPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func brainyxManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".brainyx"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func brainyxExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectBrainyxExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("brainyx.mjs").path,
            root.appendingPathComponent("bin/brainyx.js").path,
        ]
    ...
    static func hasAnyBrainyxInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.brainyxExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func brainyxNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let brainyxPath = self.projectBrainyxExecutable(projectRoot: root) {
                return [brainyxPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "brainyx", subcommand] + extraArgs
            }
            if let brainyxPath = self.brainyxExecutable(searchPaths: searchPaths) {
                return [brainyxPath, subcommand] + extraArgs
            }

            let missingEntry = """
            brainyx entrypoint missing (looked for dist/index.js or brainyx.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real brainyx CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/brainyx"
        ...
        \(cliSection)
        \(projectSection)
        if command -v brainyx >/dev/null 2>&1; then
          CLI="$(command -v brainyx)"
          brainyx \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/brainyx.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/brainyx.mjs"
            node "$PRJ/brainyx.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/brainyx.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/brainyx.js"
            node "$PRJ/bin/brainyx.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent brainyx"
          pnpm --silent brainyx \(quotedArgs);
        else
          echo "brainyx CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? BrainyxConfigFile.loadDict()
