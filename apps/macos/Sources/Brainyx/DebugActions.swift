...
enum DebugActions {
    private static let verboseDefaultsKey = "brainyx.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "brainyx.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = BrainyxPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Brainyx", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = BrainyxPaths.configURL
        guard
