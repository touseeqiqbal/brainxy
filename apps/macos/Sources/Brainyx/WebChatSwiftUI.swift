...
private let webChatSwiftLogger = Logger(subsystem: "ai.brainyx", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: BrainyxChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> BrainyxChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(BrainyxChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> BrainyxChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                BrainyxGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<BrainyxChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any BrainyxChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = BrainyxChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: BrainyxChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<BrainyxChatView>) -> NSViewController
    {
