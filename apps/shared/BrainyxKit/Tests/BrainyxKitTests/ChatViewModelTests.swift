...
private final class TestChatTransport: @unchecked Sendable, BrainyxChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [BrainyxChatHistoryPayload]
    private let sessionsResponses: [BrainyxChatSessionsListResponse]

    private let stream: AsyncStream<BrainyxChatTransportEvent>
    private let continuation: AsyncStream<BrainyxChatTransportEvent>.Continuation

    init(
        historyResponses: [BrainyxChatHistoryPayload],
        sessionsResponses: [BrainyxChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<BrainyxChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> BrainyxChatHistoryPayload {
        ...
        return self.historyResponses.last ?? BrainyxChatHistoryPayload(
        ...
        attachments _: [BrainyxChatAttachmentPayload]) async throws -> BrainyxChatSendResponse
    {
        ...
        return BrainyxChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> BrainyxChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? BrainyxChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: BrainyxChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = BrainyxChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = BrainyxChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { BrainyxChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = BrainyxChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = BrainyxChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = BrainyxChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { BrainyxChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
