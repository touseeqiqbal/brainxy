...
struct ChatMessageBubble: View {
    let message: BrainyxChatMessage
    let style: BrainyxChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? BrainyxChatTheme.userText : BrainyxChatTheme.assistantText
    ...
    private var inlineAttachments: [BrainyxChatMessageContent] {
    ...
    private var toolCalls: [BrainyxChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [BrainyxChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? BrainyxChatTheme.userBubble
        }
        if self.style == .onboarding {
            return BrainyxChatTheme.onboardingAssistantBubble
        }
        return BrainyxChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return BrainyxChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: BrainyxChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? BrainyxChatTheme.userText : BrainyxChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: BrainyxChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(BrainyxChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(BrainyxChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [BrainyxChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: BrainyxChatTheme.assistantText)
            }
        }
    }
}
