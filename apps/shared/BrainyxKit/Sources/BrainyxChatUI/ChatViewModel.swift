...
private let chatUILogger = Logger(subsystem: "ai.brainyx", category: "BrainyxChatUI")

@MainActor
@Observable
public final class BrainyxChatViewModel {
    public private(set) var messages: [BrainyxChatMessage] = []
    ...
    public var attachments: [BrainyxPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [BrainyxChatPendingToolCall] = []
    public private(set) var sessions: [BrainyxChatSessionEntry] = []
    private let transport: any BrainyxChatTransport
    ...
    private var pendingToolCallsById: [String: BrainyxChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any BrainyxChatTransport) {
    ...
    public var sessionChoices: [BrainyxChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [BrainyxChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [BrainyxChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: BrainyxPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [BrainyxChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: BrainyxChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [BrainyxChatMessage]) -> [BrainyxChatMessage] {
        var result: [BrainyxChatMessage] = []
    ...
    private static func dedupeKey(for message: BrainyxChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [BrainyxChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> BrainyxChatAttachmentPayload in
            BrainyxChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> BrainyxChatSessionEntry {
        BrainyxChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: BrainyxChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: BrainyxChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: BrainyxAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = BrainyxChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> BrainyxPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
