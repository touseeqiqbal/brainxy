## Brainyx Node (Android) (internal)

Modern Android node app: connects to the **Gateway WebSocket** (`_brainyx-gw._tcp`) and exposes **Canvas + Chat + Camera**.
...
```bash
pnpm brainyx gateway --port 18789 --verbose
```
...
```bash
brainyx nodes pending
brainyx nodes approve <requestId>
```
