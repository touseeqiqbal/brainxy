...
struct IOSGatewayChatTransport: BrainyxChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> BrainyxChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(BrainyxChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> BrainyxChatHistoryPayload {
        ...
        return try JSONDecoder().decode(BrainyxChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [BrainyxChatAttachmentPayload]) async throws -> BrainyxChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [BrainyxChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(BrainyxChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(BrainyxGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: BrainyxGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: BrainyxChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: BrainyxAgentEventPayload.self)
                        {
