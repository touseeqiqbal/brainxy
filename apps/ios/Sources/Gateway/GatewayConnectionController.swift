...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [BrainyxCapability.canvas.rawValue, BrainyxCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(BrainyxCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(BrainyxCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = BrainyxLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(BrainyxCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            BrainyxCanvasCommand.present.rawValue,
            ...
            BrainyxCanvasA2UICommand.reset.rawValue,
            BrainyxScreenCommand.record.rawValue,
            BrainyxSystemCommand.notify.rawValue,
            BrainyxSystemCommand.which.rawValue,
            BrainyxSystemCommand.run.rawValue,
            BrainyxSystemCommand.execApprovalsGet.rawValue,
            BrainyxSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(BrainyxCapability.camera.rawValue) {
            commands.append(BrainyxCameraCommand.list.rawValue)
            commands.append(BrainyxCameraCommand.snap.rawValue)
            commands.append(BrainyxCameraCommand.clip.rawValue)
        }
        if caps.contains(BrainyxCapability.location.rawValue) {
            commands.append(BrainyxLocationCommand.get.rawValue)
        }
