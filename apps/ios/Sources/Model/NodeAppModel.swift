...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = BrainyxCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = BrainyxCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = BrainyxCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = BrainyxCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = BrainyxCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__brainyx__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: BrainyxLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for brainyx:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case BrainyxLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case BrainyxCanvasCommand.present.rawValue,
                 BrainyxCanvasCommand.hide.rawValue,
                 BrainyxCanvasCommand.navigate.rawValue,
                 BrainyxCanvasCommand.evalJS.rawValue,
                 BrainyxCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case BrainyxCanvasA2UICommand.reset.rawValue,
                 BrainyxCanvasA2UICommand.push.rawValue,
                 BrainyxCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case BrainyxCameraCommand.list.rawValue,
                 BrainyxCameraCommand.snap.rawValue,
                 BrainyxCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case BrainyxScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: BrainyxNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: BrainyxNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(BrainyxLocationGetParams.self, from: req.paramsJSON)) ??
            BrainyxLocationGetParams()
        ...
        let payload = BrainyxLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case BrainyxCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(BrainyxCanvasPresentParams.self, from: req.paramsJSON)) ??
                BrainyxCanvasPresentParams()
        ...
        case BrainyxCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case BrainyxCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(BrainyxCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case BrainyxCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(BrainyxCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case BrainyxCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(BrainyxCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case BrainyxCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.brainyxA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing brainyxA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case BrainyxCanvasA2UICommand.push.rawValue, BrainyxCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == BrainyxCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(BrainyxCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try BrainyxCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(BrainyxCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try BrainyxCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.brainyxA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case BrainyxCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case BrainyxCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(BrainyxCameraSnapParams.self, from: req.paramsJSON)) ??
                BrainyxCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case BrainyxCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(BrainyxCameraClipParams.self, from: req.paramsJSON)) ??
                BrainyxCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(BrainyxScreenRecordParams.self, from: req.paramsJSON)) ??
            BrainyxScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> BrainyxLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return BrainyxLocationMode(rawValue: raw) ?? .off
    }
