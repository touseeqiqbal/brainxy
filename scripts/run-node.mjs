...
// Auto-inject dev environment variables if --dev flag is present
// Channels are enabled by default now to allow configuration via UI.

const cwd = process.cwd();
const compiler = env.BRAINYX_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.BRAINYX_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.BRAINYX_RUNNER_LOG === "0") return;
  process.stderr.write(`[brainyx] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["brainyx.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
...
if (!shouldBuild()) {
  ...
  const buildArgs =
    process.platform === "win32" ? ["/d", "/s", "/c", "npx", "pnpm", ...pnpmArgs] : pnpmArgs;
