...
function resolveBrainyxRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.BRAINYX_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set BRAINYX_ROOT to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveBrainyxRoot();
