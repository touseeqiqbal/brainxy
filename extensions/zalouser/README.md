...
Brainyx extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
brainyx channels login --channel zalouser
...
```bash
brainyx message send --channel zalouser --target <threadId> --message "Hello from Brainyx!"
```
...
```bash
brainyx channels login --channel zalouser              # Login via QR
brainyx channels login --channel zalouser --account work
brainyx channels status --probe
brainyx channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
brainyx directory self --channel zalouser
brainyx directory peers list --channel zalouser --query "name"
brainyx directory groups list --channel zalouser --query "work"
brainyx directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
brainyx message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
brainyx message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `brainyx channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
brainyx channels login --channel zalouser --account work
brainyx message send --channel zalouser --account work --target <id> --message "Hello"
