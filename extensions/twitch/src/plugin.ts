/**
 * Twitch channel plugin for Brainyx.
 *
...
import { buildChannelConfigSchema } from "brainyx/plugin-sdk";
 ...
 * for Brainyx. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: BrainyxConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: BrainyxConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: BrainyxConfig): boolean => {
