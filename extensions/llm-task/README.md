...
`brainyx.invoke --each`) without adding new Brainyx code per workflow.
...
This extension depends on Brainyx internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Brainyx extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.brainyx/extensions` as a standalone plugin directory.
