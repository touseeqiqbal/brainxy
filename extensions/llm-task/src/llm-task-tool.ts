...
// NOTE: This extension is intended to be bundled with Brainyx.
// When running from source (tests/dev), Brainyx internals live under src/.
...
export function createLlmTaskTool(api: BrainyxPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via brainyx.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-llm-task-"));
