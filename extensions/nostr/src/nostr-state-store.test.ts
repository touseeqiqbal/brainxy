...
import type { PluginRuntime } from "brainyx/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.BRAINYX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-nostr-"));
  process.env.BRAINYX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override =
          env.BRAINYX_STATE_DIR?.trim() || env.BRAINYX_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".brainyx");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.BRAINYX_STATE_DIR;
    else process.env.BRAINYX_STATE_DIR = previous;
