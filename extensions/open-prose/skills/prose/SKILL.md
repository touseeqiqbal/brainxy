...
metadata: {"brainyx":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
## Brainyx Runtime Mapping

- **Task tool** in the upstream spec == Brainyx `sessions_spawn`
- **File I/O** == Brainyx `read`/`write`
- **Remote fetch** == Brainyx `web_fetch` (or `exec` with curl when POST is required)
