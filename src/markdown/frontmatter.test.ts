...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { brainyx?: { emoji?: string } };
    expect(parsed.brainyx?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { brainyx?: { events?: string[] } };
