...
type AgentConfig = Partial<NonNullable<NonNullable<BrainyxConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.brainyx/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Brainyx ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: BrainyxConfig): string {
  const lines = ["ℹ️ Help", ""];
