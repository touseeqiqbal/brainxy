...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-commands-"));
...
function buildParams(commandBody: string, cfg: BrainyxConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
