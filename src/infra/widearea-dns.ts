...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.BRAINYX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*brainyx-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "brainyx";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "brainyx");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "brainyx-gw");
  ...
  records.push(`_brainyx-gw._tcp IN PTR ${instanceLabel}._brainyx-gw._tcp`);
  records.push(`${instanceLabel}._brainyx-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._brainyx-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; brainyx-content-hash: ${contentHash}\n${contentBody}`;
}
