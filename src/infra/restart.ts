...
export function triggerBrainyxRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.BRAINYX_SYSTEMD_UNIT,
        process.env.BRAINYX_PROFILE,
      );
  ...
  const label =
    process.env.BRAINYX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.BRAINYX_PROFILE);
