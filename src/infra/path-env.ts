...
function candidateBinDirs(opts: EnsureBrainyxPathOpts): string[] {
  ...
  // Bundled macOS app: `brainyx` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const candidateCli = path.join(execDir, "brainyx");
    if (isExecutable(candidateCli)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/brainyx` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "brainyx"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `brainyx` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureBrainyxCliOnPath(opts: EnsureBrainyxPathOpts = {}) {
  if (isTruthyEnvValue(process.env.BRAINYX_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.BRAINYX_PATH_BOOTSTRAPPED = "1";
