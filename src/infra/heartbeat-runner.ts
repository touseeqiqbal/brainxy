...
function resolveActiveHoursTimezone(cfg: BrainyxConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: BrainyxConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: BrainyxConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: BrainyxConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: BrainyxConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: BrainyxConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: BrainyxConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: BrainyxConfig) => {
    if (state.stopped) return;
    const now = Date.now();
