...
function setWhatsAppDmPolicy(cfg: BrainyxConfig, dmPolicy: DmPolicy): BrainyxConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: BrainyxConfig, allowFrom?: string[]): BrainyxConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: BrainyxConfig, selfChatMode: boolean): BrainyxConfig {
...
async function detectWhatsAppLinked(cfg: BrainyxConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Brainyx can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Brainyx" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("brainyx channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
