...
function setSlackDmPolicy(cfg: BrainyxConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Brainyx";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Brainyx`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/brainyx",
          description: "Send a message to Brainyx",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: BrainyxConfig, allowFrom: string[]): BrainyxConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
