...
/**
 * Cleanup suspended Brainyx CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: BrainyxConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-cli-images-"));
  const paths: string[] = [];
