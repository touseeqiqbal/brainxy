...
const defaultTools = createBrainyxCodingTools();

describe("createBrainyxCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createBrainyxCodingTools();
    ...
    try {
      ...
      const contents = "Hello from brainyx read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "brainyx-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "brainyx-workspace"),
    ...
    const tools = createBrainyxCodingTools({ sandbox });
