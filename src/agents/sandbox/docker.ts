...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "brainyx.sandbox=1");
  args.push("--label", `brainyx.sessionKey=${params.scopeKey}`);
  args.push("--label", `brainyx.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `brainyx.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("brainyx.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`brainyx sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`brainyx sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("brainyx sandbox recreate --all");
}
