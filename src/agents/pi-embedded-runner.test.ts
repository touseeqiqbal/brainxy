...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-embedded-agent-"));
...
const ensureModels = (cfg: BrainyxConfig) => ensureBrainyxModelsJson(cfg, agentDir) as unknown;
