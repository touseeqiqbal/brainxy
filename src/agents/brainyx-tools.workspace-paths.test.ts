...
import { createBrainyxCodingTools } from "./brainyx-coding-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("brainyx-ws-", async (workspaceDir) => {
      await withTempDir("brainyx-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createBrainyxCodingTools({ workspaceDir });
    ...
    await withTempDir("brainyx-ws-", async (workspaceDir) => {
      await withTempDir("brainyx-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello brainyx");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("brainyx-ws-", async (workspaceDir) => {
      await withTempDir("brainyx-override-", async (overrideDir) => {
        const tools = createBrainyxCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("brainyx-sandbox-", async (sandboxDir) => {
      await withTempDir("brainyx-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "brainyx-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "brainyx-sandbox:bookworm-slim",
            containerPrefix: "brainyx-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createBrainyxCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
