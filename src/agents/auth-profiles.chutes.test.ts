...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.BRAINYX_AGENT_DIR;
    else process.env.BRAINYX_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-chutes-"));
    process.env.BRAINYX_STATE_DIR = tempDir;
    process.env.BRAINYX_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
