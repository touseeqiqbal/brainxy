...
import { createBrainyxTools } from "./brainyx-tools.js";
import { __testing, createBrainyxCodingTools } from "./brainyx-coding-tools.js";
import { createSandboxedReadTool } from "./brainyx-tools.read.js";
...
describe("createBrainyxCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createBrainyxTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createBrainyxCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "brainyx-outside.txt");
