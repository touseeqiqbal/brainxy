...
export function listSignalAccountIds(cfg: BrainyxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: BrainyxConfig): string {
...
function mergeSignalAccountConfig(cfg: BrainyxConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: BrainyxConfig): ResolvedSignalAccount[] {
