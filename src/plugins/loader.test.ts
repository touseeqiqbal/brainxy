...
import { loadBrainyxPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.BRAINYX_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `brainyx-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "brainyx.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.BRAINYX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadBrainyxPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.BRAINYX_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadBrainyxPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadBrainyxPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@brainyx/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        brainyx: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
