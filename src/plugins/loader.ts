...
import { discoverBrainyxPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: BrainyxPluginDefinition;
  register?: BrainyxPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as BrainyxPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as BrainyxPluginDefinition;
...
export function loadBrainyxPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverBrainyxPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "brainyx/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: BrainyxPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as BrainyxPluginModule;
    } catch (err) {
