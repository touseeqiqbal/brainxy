...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.BRAINYX_STATE_DIR;
  const configPath = env.BRAINYX_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    BRAINYX_PROFILE: profile,
    BRAINYX_STATE_DIR: stateDir,
    BRAINYX_CONFIG_PATH: configPath,
    BRAINYX_GATEWAY_PORT: String(port),
    BRAINYX_GATEWAY_TOKEN: token,
    BRAINYX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    BRAINYX_SYSTEMD_UNIT: systemdUnit,
    BRAINYX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    BRAINYX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    BRAINYX_SERVICE_VERSION: VERSION,
  };
}
