...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.BRAINYX_GATEWAY_PORT).toBe("18789");
    expect(env.BRAINYX_GATEWAY_TOKEN).toBe("secret");
    expect(env.BRAINYX_SERVICE_MARKER).toBe("brainyx");
    expect(env.BRAINYX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.BRAINYX_SERVICE_VERSION).toBe("string");
    expect(env.BRAINYX_SYSTEMD_UNIT).toBe("brainyx-gateway.service");
    if (process.platform === "darwin") {
      expect(env.BRAINYX_LAUNCHD_LABEL).toBe("ai.brainyx.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", BRAINYX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.BRAINYX_SYSTEMD_UNIT).toBe("brainyx-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.BRAINYX_LAUNCHD_LABEL).toBe("ai.brainyx.work");
    }
  });
});
