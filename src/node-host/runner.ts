...
const execHostEnforced = process.env.BRAINYX_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.BRAINYX_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureBrainyxCliOnPath({ pathEnv: process.env.PATH ?? "" });
