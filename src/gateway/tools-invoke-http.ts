...
} from "../agents/brainyx-tools.policy.js";
import {
...
import { filterToolsByRole } from "./rbac.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-brainyx-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-brainyx-account-id")?.trim() || undefined;
  ...
  const allTools = createBrainyxTools({
  ...
  const startTools = filterToolsByRole(allTools, role);
  ...
  const toolsFiltered = profilePolicyExpanded
    ? filterToolsByPolicy(startTools, profilePolicyExpanded)
    : startTools;
