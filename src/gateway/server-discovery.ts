...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Brainyx";
  if (/brainyx/i.test(trimmed)) return trimmed;
  return `${trimmed} (Brainyx)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.BRAINYX_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "brainyx");
  ...
  const binCli = path.join(cwd, "bin", "brainyx");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.BRAINYX_TAILNET_DNS?.trim();
