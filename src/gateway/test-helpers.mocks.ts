...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `brainyx-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.BRAINYX_CONFIG_PATH = path.join(root, "brainyx.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "brainyx.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "brainyx-gateway-test"),
        ...fileDefaults,
...
vi.mock("../agents/brainyx-embedded.js", async () => {
  const actual = await vi.importActual<typeof import("../agents/brainyx-embedded.js")>(
    "../agents/brainyx-embedded.js",
  );
  return {
    ...actual,
