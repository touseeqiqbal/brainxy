...
import { createBrainyxTools } from "../agents/brainyx-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.BRAINYX_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.BRAINYX_GATEWAY_PORT = String(gatewayPort);
  process.env.BRAINYX_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.BRAINYX_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createBrainyxTools().find((candidate) => candidate.name === "sessions_send");
