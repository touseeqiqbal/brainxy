...
beforeAll(async () => {
  previousToken = process.env.BRAINYX_GATEWAY_TOKEN;
...
afterAll(async () => {
  await server.close();
  if (previousToken === undefined) delete process.env.BRAINYX_GATEWAY_TOKEN;
  else process.env.BRAINYX_GATEWAY_TOKEN = previousToken;
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
