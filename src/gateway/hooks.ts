...
export function resolveHooksConfig(cfg: BrainyxConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-brainyx-token"] === "string"
      ? req.headers["x-brainyx-token"].trim()
      : "";
