...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.BRAINYX_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "brainyx");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.brainyx-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".brainyx");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "brainyx.json");
      ...
      process.env.BRAINYX_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.BRAINYX_CONFIG_PATH = prev.configPath;
        process.env.BRAINYX_GATEWAY_TOKEN = prev.token;
        process.env.BRAINYX_SKIP_CHANNELS = prev.skipChannels;
        process.env.BRAINYX_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.BRAINYX_SKIP_CRON = prev.skipCron;
        process.env.BRAINYX_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.BRAINYX_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.BRAINYX_STATE_DIR = prev.stateDir;
      process.env.BRAINYX_CONFIG_PATH = prev.configPath;
      process.env.BRAINYX_GATEWAY_TOKEN = prev.token;
      process.env.BRAINYX_SKIP_CHANNELS = prev.skipChannels;
      process.env.BRAINYX_SKIP_GMAIL_WATCHER = prev.skipGmail;
