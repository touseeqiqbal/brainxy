...
import { resolveBrainyxAgentDir } from "../agents/agent-paths.js";
...
import { ensureBrainyxModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { BrainyxConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.BRAINYX_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.BRAINYX_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.BRAINYX_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<BrainyxConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.BRAINYX_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.BRAINYX_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveBrainyxAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-live-state-"));
  ...
  process.env.BRAINYX_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.brainyx-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveBrainyxAgentDir();
  const sanitizedCfg: BrainyxConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Brainyx live tool probe (local, safe): " +
    ...
    process.env.BRAINYX_CONFIG_PATH = previous.configPath;
    process.env.BRAINYX_GATEWAY_TOKEN = previous.token;
    process.env.BRAINYX_SKIP_CHANNELS = previous.skipChannels;
    process.env.BRAINYX_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.BRAINYX_SKIP_CRON = previous.skipCron;
    process.env.BRAINYX_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.BRAINYX_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.BRAINYX_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureBrainyxModelsJson(cfg);
      ...
      const rawModels = process.env.BRAINYX_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.brainyx-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
