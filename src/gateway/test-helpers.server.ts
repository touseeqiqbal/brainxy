...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.BRAINYX_STATE_DIR;
  previousConfigPath = process.env.BRAINYX_CONFIG_PATH;
  previousSkipBrowserControl = process.env.BRAINYX_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.BRAINYX_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.BRAINYX_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.BRAINYX_STATE_DIR = path.join(tempHome, ".brainyx");
  delete process.env.BRAINYX_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.BRAINYX_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "brainyx-test-"))
    : path.join(tempHome, ".brainyx-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.BRAINYX_STATE_DIR;
    else process.env.BRAINYX_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.BRAINYX_CONFIG_PATH;
    else process.env.BRAINYX_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.BRAINYX_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.BRAINYX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.BRAINYX_SKIP_GMAIL_WATCHER;
    else process.env.BRAINYX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.BRAINYX_SKIP_CANVAS_HOST;
    else process.env.BRAINYX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.BRAINYX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.BRAINYX_GATEWAY_TOKEN = fallbackToken;
  }
