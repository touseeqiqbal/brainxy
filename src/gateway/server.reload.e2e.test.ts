...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.BRAINYX_SKIP_CHANNELS;
    prevSkipGmail = process.env.BRAINYX_SKIP_GMAIL_WATCHER;
    process.env.BRAINYX_SKIP_CHANNELS = "0";
    delete process.env.BRAINYX_SKIP_GMAIL_WATCHER;
  });

  afterEach(() => {
    if (prevSkipChannels === undefined) {
      delete process.env.BRAINYX_SKIP_CHANNELS;
    } else {
      process.env.BRAINYX_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.BRAINYX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.BRAINYX_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
