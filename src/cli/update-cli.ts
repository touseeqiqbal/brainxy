...
import { resolveBrainyxPackageRoot } from "../infra/brainyx-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "brainyx doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "brainyx";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const BRAINYX_REPO_URL = "https://github.com/brainyx/brainyx.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".brainyx");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("brainyx@")) return trimmed.slice("brainyx@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.BRAINYX_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", BRAINYX_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `BRAINYX_GIT_DIR points at a non-git directory: ${params.dir}. Set BRAINYX_GIT_DIR to an empty folder or an brainyx checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`BRAINYX_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveBrainyxPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Brainyx update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Brainyx..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Brainyx install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("brainyx doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("brainyx gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g brainyx@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g brainyx@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.BRAINYX_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.BRAINYX_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("brainyx gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("brainyx doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("brainyx gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("brainyx gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `brainyx update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `BRAINYX_GIT_DIR points at a non-git directory: ${gitDir}. Set BRAINYX_GIT_DIR to an empty folder or an brainyx checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via BRAINYX_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Brainyx to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["brainyx update", "Update a source checkout (git)"],
        ["brainyx update --channel beta", "Switch to beta channel (git + npm)"],
        ["brainyx update --channel dev", "Switch to dev channel (git + npm)"],
        ["brainyx update --tag beta", "One-off update to a dist-tag or version"],
        ["brainyx update --no-restart", "Update without restarting the service"],
        ["brainyx update --json", "Output result as JSON"],
        ["brainyx update --yes", "Non-interactive (accept downgrade prompts)"],
        ["brainyx update wizard", "Interactive update wizard"],
        ["brainyx --update", "Shorthand for brainyx update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run brainyx update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.brainyx.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brainyx update status", "Show channel + version status."],
          ["brainyx update status --json", "JSON output."],
          ["brainyx update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.brainyx.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
