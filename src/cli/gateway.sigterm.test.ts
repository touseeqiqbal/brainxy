...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "brainyx-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      BRAINYX_NO_RESPAWN: "1",
      BRAINYX_STATE_DIR: stateDir,
      BRAINYX_SKIP_CHANNELS: "1",
      BRAINYX_SKIP_GMAIL_WATCHER: "1",
      BRAINYX_SKIP_CRON: "1",
      BRAINYX_SKIP_BROWSER_CONTROL_SERVER: "1",
      BRAINYX_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "brainyx-entry-bootstrap.mjs");
