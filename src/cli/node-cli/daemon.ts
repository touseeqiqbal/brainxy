...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("brainyx node install"), formatCliCommand("brainyx node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("brainyx node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    BRAINYX_LOG_PREFIX: baseEnv.BRAINYX_LOG_PREFIX ?? "node",
