...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    BRAINYX_STATE_DIR: process.env.BRAINYX_STATE_DIR,
    BRAINYX_CONFIG_PATH: process.env.BRAINYX_CONFIG_PATH,
    BRAINYX_GATEWAY_PORT: process.env.BRAINYX_GATEWAY_PORT,
    BRAINYX_PROFILE: process.env.BRAINYX_PROFILE,
  };

  beforeEach(() => {
    process.env.BRAINYX_STATE_DIR = "/tmp/brainyx-cli-state";
    process.env.BRAINYX_CONFIG_PATH = "/tmp/brainyx-cli-state/brainyx.json";
    delete process.env.BRAINYX_GATEWAY_PORT;
    delete process.env.BRAINYX_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.BRAINYX_STATE_DIR !== undefined)
      process.env.BRAINYX_STATE_DIR = originalEnv.BRAINYX_STATE_DIR;
    else delete process.env.BRAINYX_STATE_DIR;

    if (originalEnv.BRAINYX_CONFIG_PATH !== undefined)
      process.env.BRAINYX_CONFIG_PATH = originalEnv.BRAINYX_CONFIG_PATH;
    else delete process.env.BRAINYX_CONFIG_PATH;

    if (originalEnv.BRAINYX_GATEWAY_PORT !== undefined)
      process.env.BRAINYX_GATEWAY_PORT = originalEnv.BRAINYX_GATEWAY_PORT;
    else delete process.env.BRAINYX_GATEWAY_PORT;

    if (originalEnv.BRAINYX_PROFILE !== undefined)
      process.env.BRAINYX_PROFILE = originalEnv.BRAINYX_PROFILE;
    else delete process.env.BRAINYX_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        BRAINYX_PROFILE: "dev",
        BRAINYX_STATE_DIR: "/tmp/brainyx-daemon-state",
        BRAINYX_CONFIG_PATH: "/tmp/brainyx-daemon-state/brainyx.json",
        BRAINYX_GATEWAY_PORT: "19001",
      },
