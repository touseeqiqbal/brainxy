...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.brainyx${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.BRAINYX_PROFILE = profile;

  const stateDir = env.BRAINYX_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.BRAINYX_STATE_DIR?.trim()) env.BRAINYX_STATE_DIR = stateDir;

  if (!env.BRAINYX_CONFIG_PATH?.trim()) {
    env.BRAINYX_CONFIG_PATH = path.join(stateDir, "brainyx.json");
  }

  if (profile === "dev" && !env.BRAINYX_GATEWAY_PORT?.trim()) {
    env.BRAINYX_GATEWAY_PORT = "19001";
  }
}
