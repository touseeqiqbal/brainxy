...
${formatHelpExamples([
  ['brainyx agent --to +15555550123 --message "status update"', "Start a new session."],
  ['brainyx agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'brainyx agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'brainyx agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['brainyx agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'brainyx agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.brainyx.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.brainyx.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['brainyx agents set-identity --agent main --name "Brainyx" --emoji "🦞"', "Set name + emoji."],
  ["brainyx agents set-identity --agent main --avatar avatars/brainyx.png", "Set avatar path."],
  [
    "brainyx agents set-identity --workspace ~/.brainyx/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "brainyx agents set-identity --identity-file ~/.brainyx/workspace/IDENTITY.md --agent main",
