...
export function listIMessageAccountIds(cfg: BrainyxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: BrainyxConfig): string {
...
function mergeIMessageAccountConfig(cfg: BrainyxConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: BrainyxConfig): ResolvedIMessageAccount[] {
