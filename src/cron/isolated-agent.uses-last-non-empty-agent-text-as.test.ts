...
import { runEmbeddedPiAgent } from "../agents/brainyx-embedded.js";
import { runCronIsolatedAgentTurn } from "./isolated-agent.js";

async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "brainyx-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".brainyx", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<BrainyxConfig> = {},
): BrainyxConfig {
  const base: BrainyxConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "brainyx"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".brainyx", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
