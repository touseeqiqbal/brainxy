...
export function resolveConfigDir(
  ...
  const override = env.BRAINYX_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) return resolveUserPath(override);
  const newDir = path.join(homedir(), ".brainyx");
  try {
...
// Configuration root; can be overridden via BRAINYX_STATE_DIR.
