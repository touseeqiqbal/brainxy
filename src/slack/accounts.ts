...
export function listSlackAccountIds(cfg: BrainyxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: BrainyxConfig): string {
...
function mergeSlackAccountConfig(cfg: BrainyxConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: BrainyxConfig): ResolvedSlackAccount[] {
