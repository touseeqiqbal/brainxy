...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "BRAINYX_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.BRAINYX_STATE_DIR = path.join(home, ".brainyx");
    ...
    await fs.mkdir(path.join(home, ".brainyx"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.brainyx${path.sep}media`);
