...
import { BrainyxSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof BrainyxSchema.toJSONSchema>;
...
const FIELD_LABELS: Record<string, string> = {
  ...
  "gateway.auth.users": "Authorized Users (RBAC)",
  "gateway.auth.users[].username": "Username",
  "gateway.auth.users[].role": "Role",
  "gateway.auth.users[].password": "Password (Optional)",
  "gateway.auth.users[].token": "Token (Optional)",
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Brainyx writes the config.",
  ...
  "gateway.auth.users": "List of users allowed to access the gateway when 'rbac' mode is enabled.",
  "gateway.auth.users[].role": "Level of access: 'viewer' (read-only), 'editor' (can run tools), or 'admin' (full access).",
  "gateway.auth.users[].password": "Password for authentication (can be used alongside token).",
  "gateway.auth.users[].token": "Token for authentication (alternative to password).",
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /brainyx).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $BRAINYX_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.brainyx/memory/{agentId}.sqlite).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `brainyx plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.brainyx/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/brainyx",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/brainyx.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = BrainyxSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "BrainyxConfig";
