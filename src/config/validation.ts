...
import type { BrainyxConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: BrainyxConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: BrainyxConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = BrainyxSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as BrainyxConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as BrainyxConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as BrainyxConfig)),
    ),
  };
}
