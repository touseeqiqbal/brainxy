...
const ETC_BRAINYX_DIR = path.join(ROOT_DIR, "etc", "brainyx");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "brainyx.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcBrainyxPath(...parts: string[]) {
  return path.join(ETC_BRAINYX_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcBrainyxPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "brainyx.json"))).toEqual({
      shared: true,
    });
  });
});
