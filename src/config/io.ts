...
import type { BrainyxConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareBrainyxVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "BRAINYX_GATEWAY_TOKEN",
  "BRAINYX_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): BrainyxConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as BrainyxConfig;
}
...
function stampConfigVersion(cfg: BrainyxConfig): BrainyxConfig {
...
function warnIfConfigFromFuture(cfg: BrainyxConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareBrainyxVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Brainyx (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: BrainyxConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): BrainyxConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as BrainyxConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as BrainyxConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: BrainyxConfig) {
    clearConfigCache();
...
// module scope. `BRAINYX_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.BRAINYX_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.BRAINYX_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): BrainyxConfig {
...
export async function writeConfigFile(cfg: BrainyxConfig): Promise<void> {
  clearConfigCache();
