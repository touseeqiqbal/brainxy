import type { BrainyxConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: BrainyxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: BrainyxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: BrainyxConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: BrainyxConfig): boolean {
...
function isIMessageConfigured(cfg: BrainyxConfig): boolean {
...
function isWhatsAppConfigured(cfg: BrainyxConfig): boolean {
...
function isGenericChannelConfigured(cfg: BrainyxConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: BrainyxConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: BrainyxConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: BrainyxConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: BrainyxConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: BrainyxConfig, pluginId: string): BrainyxConfig {
...
function enablePluginEntry(cfg: BrainyxConfig, pluginId: string): BrainyxConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
