...
homepage: https://docs.brainyx.ai/hooks#command-logger
metadata:
  {
    "brainyx":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.brainyx/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.brainyx/logs/commands.log ~/.brainyx/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/brainyx`:
   ```
   /home/username/.brainyx/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.brainyx/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.brainyx/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.brainyx/logs/commands.log | jq .
```
