...
This directory contains hooks that ship with Brainyx. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.brainyx/workspace`)

**Enable**:

```bash
brainyx hooks enable session-memory
```
...
**Output**: `~/.brainyx/logs/commands.log`

**Enable**:

```bash
brainyx hooks enable command-logger
```
...
**Docs**: https://docs.brainyx.ai/hooks/soul-evil

**Enable**:

```bash
brainyx hooks enable soul-evil
```
...
homepage: https://docs.brainyx.ai/hooks#my-hook
metadata:
  { "brainyx": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.brainyx/hooks/` (shared across workspaces)
...
```bash
brainyx hooks info session-memory
```
...
```bash
brainyx hooks enable session-memory
brainyx hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.brainyx/brainyx.json`:
...
2. Restart gateway: `pkill -9 -f 'brainyx.*gateway' && pnpm brainyx gateway`
3. Enable the hook: `brainyx hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.brainyx.ai/hooks
