...
`;
    ...
    expect(parsed.brainyx.emoji).toBe("💾");
    expect(parsed.brainyx.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.brainyx.emoji).toBe("📝");
    expect(parsed.brainyx.events).toEqual(["command"]);
    expect(parsed.brainyx.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.brainyx.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"brainyx": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"brainyx": {"events": ["test"]}}');
  });
...
describe("resolveBrainyxMetadata", () => {
  it("extracts brainyx metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveBrainyxMetadata(frontmatter);
  ...
  it("returns undefined when brainyx key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        brainyx: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Brainyx" },
            { id: "npm", kind: "npm", package: "@brainyx/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@brainyx/hook");
  });
...
homepage: https://docs.brainyx.ai/hooks#session-memory
metadata:
  {
    "brainyx":
      {
...
`;
    ...
    const brainyx = resolveBrainyxMetadata(frontmatter);
    expect(brainyx).toBeDefined();
    expect(brainyx?.emoji).toBe("💾");
    expect(brainyx?.events).toEqual(["command:new"]);
    expect(brainyx?.requires?.config).toEqual(["workspace.dir"]);
    expect(brainyx?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const brainyx = resolveBrainyxMetadata(frontmatter);
    expect(brainyx?.emoji).toBe("disk");
    expect(brainyx?.events).toEqual(["command:new"]);
  });
});
