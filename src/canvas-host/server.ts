...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.brainyxCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.brainyxCanvasA2UIAction &&
        typeof window.brainyxCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.brainyxSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("brainyx:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Brainyx node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.brainyxSendUserAction === "function"
        ? window.brainyxSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.BRAINYX_SKIP_CANVAS_HOST)) return true;
  if (isTruthyEnvValue(process.env.BRAINYX_SKIP_CANVAS_HOST)) return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".brainyx", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Brainyx Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
