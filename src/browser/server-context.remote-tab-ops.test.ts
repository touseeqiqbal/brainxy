...
vi.mock("./chrome.js", () => ({
  ...
  launchBrainyxChrome: vi.fn(async () => {
  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        brainyx: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("brainyx");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const brainyx = ctx.forProfile("brainyx");

    const opened = await brainyx.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("brainyx")?.lastTargetId).toBe("CREATED");
  });
});
