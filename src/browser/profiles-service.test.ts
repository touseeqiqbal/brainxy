...
vi.mock("./chrome.js", () => ({
  resolveBrainyxUserDataDir: vi.fn(() => "/tmp/brainyx-test/brainyx/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveBrainyxUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "brainyx-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveBrainyxUserDataDir).mockReturnValue(userDataDir);
