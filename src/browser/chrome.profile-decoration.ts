...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".brainyx-profile-decorated");
}
 ...
 */
export function decorateBrainyxProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_BRAINYX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_BRAINYX_BROWSER_COLOR).toUpperCase();
