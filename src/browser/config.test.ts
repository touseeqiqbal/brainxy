...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const brainyx = resolveProfile(resolved, "brainyx");
    expect(brainyx?.driver).toBe("brainyx");
    expect(brainyx?.cdpPort).toBe(18800);
    expect(brainyx?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from BRAINYX_GATEWAY_PORT when unset", () => {
    const prev = process.env.BRAINYX_GATEWAY_PORT;
    process.env.BRAINYX_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const brainyx = resolveProfile(resolved, "brainyx");
      expect(brainyx?.cdpPort).toBe(19012);
      expect(brainyx?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.BRAINYX_GATEWAY_PORT;
      } else {
        process.env.BRAINYX_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const brainyx = resolveProfile(resolved, "brainyx");
      expect(brainyx?.cdpPort).toBe(19022);
      expect(brainyx?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.BRAINYX_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "brainyx");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        brainyx: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("brainyx");
  });
});
