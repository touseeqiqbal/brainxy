...
function resolveDefaultAccountId(cfg: BrainyxConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: BrainyxConfig): boolean {
...
export function summarizeBindings(cfg: BrainyxConfig, bindings: AgentBinding[]): string[] {
