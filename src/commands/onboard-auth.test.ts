...
const requireAgentDir = () => {
  const agentDir = process.env.BRAINYX_AGENT_DIR;
  if (!agentDir) throw new Error("BRAINYX_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.BRAINYX_OAUTH_DIR;
  });

  it("writes auth-profiles.json under BRAINYX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-oauth-"));
    process.env.BRAINYX_STATE_DIR = tempStateDir;
    process.env.BRAINYX_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.BRAINYX_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to BRAINYX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brainyx-minimax-"));
    process.env.BRAINYX_STATE_DIR = tempStateDir;
    process.env.BRAINYX_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.BRAINYX_AGENT_DIR;
