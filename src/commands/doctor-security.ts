...
import type { BrainyxConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: BrainyxConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("brainyx security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("brainyx configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("brainyx config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("brainyx doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "brainyx config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("brainyx config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
