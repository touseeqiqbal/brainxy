...
beforeEach(() => {
  ...
  resolveBrainyxPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.BRAINYX_STATE_DIR;
  originalUpdateInProgress = process.env.BRAINYX_UPDATE_IN_PROGRESS;
  process.env.BRAINYX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "brainyx-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.BRAINYX_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.BRAINYX_UPDATE_IN_PROGRESS;
  } else {
    process.env.BRAINYX_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveBrainyxPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadBrainyxPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/brainyx-root.js", () => ({
  resolveBrainyxPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "brainyx-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.BRAINYX_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when BRAINYX_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/brainyx.json",
      exists: true,
      raw: "{}",
    ...
    process.env.BRAINYX_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
