...
import { readConfigFileSnapshot, type BrainyxConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: BrainyxConfig, model: string): BrainyxConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("brainyx plugins install")}\`.`,
    );
  }
