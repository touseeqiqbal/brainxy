...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("brainyx models auth setup-token")}\` or \`${formatCliCommand("brainyx configure")}\`.`
          : `Run \`${formatCliCommand("brainyx configure")}\` or set an API key env var.`;
