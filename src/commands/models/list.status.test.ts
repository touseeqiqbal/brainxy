...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveBrainyxAgentDir: vi.fn().mockReturnValue("/tmp/brainyx-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveBrainyxAgentDir: mocks.resolveBrainyxAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/brainyx-agent/auth-profiles.json");
