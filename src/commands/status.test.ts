...
beforeAll(() => {
  previousProfile = process.env.BRAINYX_PROFILE;
  process.env.BRAINYX_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.BRAINYX_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/brainyx-root.js", () => ({
  resolveBrainyxPackageRoot: vi.fn().mockResolvedValue("/tmp/brainyx"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/brainyx/pnpm-lock.yaml",
      markerPath: "/tmp/brainyx/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Brainyx status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("brainyx status --all") ||
          l.includes("brainyx --profile isolated status --all") ||
          l.includes("brainyx status --all") ||
          l.includes("brainyx --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.BRAINYX_GATEWAY_TOKEN = "abcd1234";
    try {
