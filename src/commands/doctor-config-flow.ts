...
function stripUnknownConfigKeys(config: BrainyxConfig): {
  config: BrainyxConfig;
  removed: string[];
} {
  const parsed = BrainyxSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as BrainyxConfig;
...
function noteOpencodeProviderOverrides(cfg: BrainyxConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".brainyx");
  const targetPath = path.join(targetDir, "brainyx.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".brainyx", "brainyx.json"),
    path.join(home, ".brainyx", "brainyx.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: BrainyxConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as BrainyxConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("brainyx doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("brainyx doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "brainyx doctor --fix" to remove these keys.');
    }
  }
