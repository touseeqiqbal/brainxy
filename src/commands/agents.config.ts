...
export function listAgentEntries(cfg: BrainyxConfig): AgentEntry[] {
...
function resolveAgentName(cfg: BrainyxConfig, agentId: string) {
...
function resolveAgentModel(cfg: BrainyxConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: BrainyxConfig): AgentSummary[] {
