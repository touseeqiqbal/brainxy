...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "BRAINYX_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("BRAINYX_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv BRAINYX_GATEWAY_TOKEN");
    expect(message).not.toContain("BRAINYX_GATEWAY_PASSWORD");
  });
