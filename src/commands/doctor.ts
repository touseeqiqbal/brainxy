...
function resolveMode(cfg: BrainyxConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Brainyx doctor");

  const root = await resolveBrainyxPackageRoot({
  ...
  let cfg: BrainyxConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("brainyx configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("brainyx config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("brainyx setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("brainyx doctor --fix")}" to apply changes.`);
  }
