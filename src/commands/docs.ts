...
const SEARCH_TOOL = "https://docs.brainyx.ai/mcp.SearchBrainyx";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.brainyx.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('brainyx docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.brainyx.ai/");
      runtime.log(`Search: ${formatCliCommand('brainyx docs "your query"')}`);
    }
    return;
  }
