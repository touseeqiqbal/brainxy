...
import type { BrainyxConfig, ConfigFileSnapshot } from "../config/config.js";
...
import { isToolAllowedByPolicies } from "../agents/brainyx-tools.policy.js";
...
function summarizeGroupPolicy(cfg: BrainyxConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: BrainyxConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep BRAINYX_STATE_DIR on a local-only volume and re-run "${formatCliCommand("brainyx security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: BrainyxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer BRAINYX_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: BrainyxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: BrainyxConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: BrainyxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: BrainyxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: BrainyxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: BrainyxConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: BrainyxConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: BrainyxConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: BrainyxConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
